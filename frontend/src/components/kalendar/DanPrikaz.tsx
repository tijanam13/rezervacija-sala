...
export function DanPrikaz({
  ...
  const nemaSala = saleFiltrirane.length === 0;

  const gridKolone = nemaSala
    ? "60px 1fr"
    : `60px repeat(${saleFiltrirane.length}, minmax(180px, 1fr))`;
    ...
    <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      {saleFiltrirane.length > 3 && (
        <>
          <button
            ...
            className="absolute top-1 left-[66px] z-30 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-fon-navy shadow-sm hover:bg-gray-50"
          >
            <ChevronLeft size={18} />
          </button>
          <button
            ...
            className="absolute top-1 right-1 z-30 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-fon-navy shadow-sm hover:bg-gray-50"
          >
      ...
      <div ref={skrolRef} className="bez-skrol-trake overflow-x-auto">
        <div className="w-max min-w-full">
          <div
            className="grid border-b-2 border-b-gray-300"
            style={{
              gridTemplateColumns: gridKolone,
              height: 44,
            }}
          >
            <div className="sticky left-0 z-20 flex items-center justify-center border-r border-r-gray-200 bg-white text-xs font-medium text-gray-600">
              Vreme
            </div>

            {nemaSala ? (
              <div className="flex items-center justify-center text-xs text-gray-400">
                Nema pronađenih sala za izabrane filtere
              </div>
            ) : (
              saleFiltrirane.map((s) => (
                <div
                  key={s.naziv}
                  className="flex items-center justify-center border-r border-r-gray-200 last:border-r-0 text-sm font-semibold text-fon-dark"
                >
                  {s.naziv}
                </div>
              ))
          ...
          >
            {satiNiz.map((sat, i) => (
              <div
                key={sat}
                className="sticky left-0 z-10 flex items-start justify-center border-r border-r-gray-200 bg-white pt-1 text-xs font-medium text-gray-600"
                style={{
                  gridColumn: 1,
                  gridRow: i + 1,
                }}
            ...
            {saleFiltrirane.map((s, i) => (
              <div
                key={`kolona-${s.naziv}`}
                className="border-r border-r-gray-100 last:border-r-0"
                style={{
                  gridColumn: i + 2,
